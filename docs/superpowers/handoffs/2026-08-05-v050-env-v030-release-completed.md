...
## 发布节奏：main 领先已发布版本是常态

v0.5.1 之后 `main` 仍会因文档与测试改动而领先标签，这不构成必须补发版本的理由。
判断标准是**改动是否会在用户那里改变行为**：

- 值得补发：影响运行行为或故障诊断的修复。#19、#20 属此类——装上旧版的人在浏览器
  版本错位时会被错误提示引向错误方向，而故障何时发生不由人定。
- 攒到下次：纯文档措辞澄清、测试增补、CI 配置。它们不改变已安装环境的行为。

按此标准，v0.5.1 之后的 README 路径澄清（说明 `compare_release_content.py` 只在仓库、
不随包分发）留在 `main`，待下次有实质改动时一并发布。
