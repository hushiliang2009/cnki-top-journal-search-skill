...
python_command=${CNKI_ENV_PYTHON:-python3}
...
touch "$transaction_dir/moved_skills" "$transaction_dir/created_skills" \
  "$transaction_dir/moved_runtimes" "$transaction_dir/created_runtimes" \
...
rollback_transaction() {
  ...
  done < "$transaction_dir/created_runtimes"
  while IFS='|' read -r path backup; do
    [ -n "$path" ] || continue
    [ -e "$path" ] && rm -rf "$path"
    [ -e "$backup" ] && mv "$backup" "$path"
  done < "$transaction_dir/moved_runtimes"
  while IFS= read -r path; do
    [ -n "$path" ] && [ -e "$path" ] && rm -rf "$path"
...
# 不过滤目录名，原地改名的 top-journal-search-lists-env.backup-* 会被当作同名同描述的
...
prepare_runtime() {
  ...
  if [ -e "$destination" ]; then
    ...
    backup="$backup_root/$leaf.backup-$timestamp"
    mv "$destination" "$backup"
    printf '%s|%s\n' "$destination" "$backup" >> "$transaction_dir/moved_runtimes"
    printf '%s\n' "$backup_root/$leaf" >> "$transaction_dir/backup_targets"
  else
    printf '%s\n' "$destination" >> "$transaction_dir/created_runtimes"
  fi
  mkdir -p "$destination"
}
...
codex_skill="$codex_home/skills/top-journal-search-lists-env"
claude_skill="$claude_home/skills/top-journal-search-lists-env"
if [ "$codex" = true ]; then
...
if [ "$codex" = true ]; then
  runtime_root="$codex_home/runtimes/cnki-search-env"
  runtime_backup_root="$codex_home/backups/runtimes"
else
  runtime_root="$claude_home/runtimes/cnki-search-env"
  runtime_backup_root="$claude_home/backups/runtimes"
fi
prepare_runtime "$runtime_root" "$runtime_backup_root"
...
"$runtime_python" -c 'import sys; sys.path.insert(0, sys.argv[1]); import cnki_search_env.mcp_server' "$installed_skill/scripts"
...
if [ "$codex" = true ]; then
  ...
  "$runtime_python" "$codex_skill/scripts/cnki_search_env/install_config.py" merge-codex --config "$codex_config" --skill-root "$codex_skill" --python "$runtime_python"
fi
if [ "$claude_code" = true ]; then
  claude_code_config="$HOME/.claude.json"
  backup_config "$claude_code_config"
  "$runtime_python" "$claude_skill/scripts/cnki_search_env/install_config.py" merge-claude --config "$claude_code_config" --skill-root "$claude_skill" --python "$runtime_python"
fi
...
if [ "$claude_desktop" = true ]; then
  ...
  "$runtime_python" "$claude_skill/scripts/cnki_search_env/install_config.py" merge-claude --config "$claude_desktop_config" --skill-root "$claude_skill" --python "$runtime_python"
fi
...
printf '%s\n' 'cnki-search-env installation completed. Restart the clients before verification.'
