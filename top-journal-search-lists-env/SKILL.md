...
浏览器 profile；**服务重启后需要重新登录**。该变量由安装器在升级时保留，重装后无须重设；写在 MCP 配置里的其余陈旧键仍会被清除。
...
| `environment_cssci` | 9 | 241 本 | `LY=` 精确枚举 + 按表达式长度分批，每批在学术期刊条件区勾选来源类别 CSSCI（`P0209`） |
| `pku_core` | 11—12（成员横跨 1—12 级） | 1987 本 | 只提交主题表达式，在学术期刊条件区勾选来源类别北大核心（`P01`），**不生成 `LY=`** |
...
两级同属一个 `pku_core` 范围，一次来源类别检索即可覆盖，结果按目录层级排序后补充。
其他层级不在覆盖范围内，应改用 `ai4scholar`。

环境 CSSCI 只是 CSSCI 的子集，仅靠来源类别收不窄到环境学科，因此刊名枚举
与来源类别两者都不能省。第7级中文集合在目录构建期固化，运行时不猜测语言。
...
页面操作顺序固定为：从知网首页进入高级检索，切换专业检索，先选择学术期刊，
再确认专业检索标签，随后设置出版年度和来源类别，最后填写表达式并提交。
选择学术期刊只限定文献类型，不改变上述四个受控范围及其期刊清单。

**来源类别不是专业检索字段。** CSSCI 固定为 `P0209`、北大核心固定为 `P01`，只在
选择学术期刊后显示的上方条件区中勾选，且必须在提交前生效；不写入专业检索
表达式或 `LY=` 子句，也不作为 MCP 的自由输入参数。控件缺失、重复或未勾选时返回
`page_contract_changed`，绝不提交未筛选检索。

专业检索可用字段固定为：`SU` 主题、`TKA` 篇关摘、`TI` 篇名、`KY` 关键词、
`AB` 摘要、`CO` 小标题、`FT` 全文、`AU` 作者、`FI` 第一作者、`RP` 通讯作者、
`AF` 作者单位、`LY` 期刊名称、`RF` 参考文献、`FU` 基金、`CLC` 中图分类号、
`SN` ISSN、`CN` CN、`DOI` DOI、`QKLM` 栏目信息、`FAF` 第一单位、`CF` 被引频次。
`YE` 不是可检索字段；年份只能填写在页面的出版年度起止控件中。
...
- `source_category_applied` 是全部已执行批次与字段的**合取**：任一批次未证实来源类别
