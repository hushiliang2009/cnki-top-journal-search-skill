...
async def current_public_theme_control(page: Any) -> Any:
    control = page.locator(CURRENT_THEME_SELECTOR)
    if await await_maybe(control.count()) != 1 or _normalize_control_text(
        await await_maybe(control.inner_text())
    ) != CURRENT_THEME_TEXT:
...
async def validate_public_theme_search_contract(page: Any) -> tuple[Any, Any]:
    await current_public_theme_control(page)
    ...
    return box, await public_theme_search_button(page)
...
async def wait_for_public_search_result_contract(page: Any) -> None:
    try:
        await await_maybe(
            page.wait_for_function(
                f'''() => Boolean(document.querySelector("{RESULT_TABLE_SELECTOR}") || document.body?.innerText?.includes("{NO_RESULTS_TEXT}"))''',
                timeout=10_000,
            )
        )
...
class PublicThemeSearchRunner:
    async def run(self, page: Any, query: str) -> int | None:
        box, button = await validate_public_theme_search_contract(page)
        await await_maybe(box.fill(query))
        async with page.expect_navigation(wait_until="domcontentloaded") as navigation:
            await await_maybe(button.click())
        await wait_for_public_search_result_contract(page)
        response = await await_maybe(navigation.value)
