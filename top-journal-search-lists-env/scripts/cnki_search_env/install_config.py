...
def cnki_server_config(skill_root: Path, python_executable: Path) -> dict[str, object]:
    ...
    if runtime_root.name.casefold() in {"scripts", "bin"}:
        virtual_environment = runtime_root.parent
        if virtual_environment.name == ".venv":
            runtime_root = virtual_environment.parent
    return {
        ...
        "env": {
            ...
            "PLAYWRIGHT_BROWSERS_PATH": str(runtime_root / "playwright-browsers"),
        },
    }
...
def _reject_unsupported_cnki_definitions(
    ...
    if root_inline_table:
        ...
        if isinstance(servers, Mapping) and "cnki-search-env" in servers:
            raise ValueError(
                "现有配置以点分键或内联表定义了 mcp_servers.cnki-search-env，"
...
def _render_codex_server_inline_entry(server_config: Mapping[str, object]) -> str:
    ...
    return (
        f'"cnki-search-env" = {{ command = {command}, args = [{rendered_args}], '
        f"env = {{ {rendered_env} }} }}"
    )
