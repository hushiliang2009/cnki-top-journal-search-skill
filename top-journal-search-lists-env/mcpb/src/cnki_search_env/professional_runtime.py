"""WebVPN 专业检索的默认生产运行时（环境版）。

本文件是通用版 cnki_search.professional_runtime 的独立移植，不导入通用版包。
环境变量与状态目录都与通用版分开，两个产品可以同时安装、各自登录、互不覆盖
断点与节流状态。
"""
...
#: 启用环境版 WebVPN 模式的环境变量。与通用版的 CNKI_WEBVPN_HOME 分开，
#: 否则设一个就把两个产品一起打开，使用者无法只启用其中一个。
...
def default_state_dir() -> Path:
    ...
    与通用版的 ~/.cnki-search 分开：两个产品的批次令牌互不相同，共用一个
    断点文件会让其中一个的续跑不断被另一个作废；节流时间戳共用则会把两个
    产品的请求间隔互相计入，表现为莫名其妙的多等 30 秒。
    """
    return Path.home() / ".cnki-search-env"
