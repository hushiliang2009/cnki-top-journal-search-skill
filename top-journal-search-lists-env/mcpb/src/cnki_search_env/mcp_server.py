...
from .professional_service import CHINESE_ENVIRONMENT_TOP_GROUP, SUPPORTED_GROUPS
...
REQUIRED_TOOLS = ["cnki_search_env", "cnki_professional_search_env"]
MIN_LIMIT = 1
#: 公开匿名模式只读结果页默认呈现的一页，从不去点「显示」档位控件，因此实际
#: 永远只有 20 行。把上限写成 50 会让调用方以为结果被截断——上限必须反映
#: 该模式真正能拿到的条数。环境版在此与通用版取同一口径。
MAX_LIMIT = 20
...
class CnkiMcpServer:
    ...
    async def cnki_professional_search_env(
        self, topic: str, group: str = CHINESE_ENVIRONMENT_TOP_GROUP,
    ...
    def build_fastmcp(self, fastmcp_class: type | None = None) -> Any:
        if fastmcp_class is None:
            ...
            mcp = FastMCP("CNKI Environmental Public Search", lifespan=lifespan)
        else:
        ...
        mcp.tool(name="cnki_search_env", description="从中国知网公开首页执行固定主题检索，并按环境期刊目录标注第一页期刊论文。")(cnki_search_env)
        ...
        async def cnki_professional_search_env(
            ...
            return await self.cnki_professional_search_env(topic, group, limit, year_from, year_to)
        ...
        mcp.tool(
            ...
            description=(
                ...
                "北大核心（第11—12级，1987本，按来源类别P01筛选）。"
                "检索字段依次为TI、SU、KY、TKA并累计合格记录；"
                "执行时先选择学术期刊，再在提交前设置出版年度和来源类别；"
                "年份与来源类别均不写入专业检索表达式。"
