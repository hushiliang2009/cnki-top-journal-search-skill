...
def test_challenge_page_is_only_observed_then_closed_before_retry(
    ...
    from cnki_search_env import professional_runtime
    from cnki_search_env.professional import ExpressionBatch
    from cnki_search_env.webvpn import CAPTCHA_VIEWPORT_JS
...
def test_challenge_wait_never_exceeds_the_600_second_limit() -> None:
    from cnki_search_env.professional import ExpressionBatch
    from cnki_search_env.professional_runtime import ProfessionalBatchExecutor
...
def test_environment_factory_requires_home_before_constructing_session(
    ...
    monkeypatch.delenv("CNKI_ENV_WEBVPN_HOME", raising=False)
    ...
    with pytest.raises(ValueError, match="请设置 CNKI_ENV_WEBVPN_HOME"):
...
# ── 与通用版的隔离（环境版专属，通用版无对应用例） ──────────────────────

def test_state_dir_is_separate_from_the_general_product() -> None:
    """两个产品的批次令牌互不相同，共用断点会让续跑被对方反复作废。

    节流时间戳共用还会把对方的请求间隔计入自己，表现为莫名其妙多等 30 秒。
    """
    from pathlib import Path

    from cnki_search_env.professional_runtime import default_state_dir

    state_dir = default_state_dir()
    assert state_dir == Path.home() / ".cnki-search-env"
    assert state_dir != Path.home() / ".cnki-search"


def test_env_product_has_its_own_opt_in_variable(monkeypatch: pytest.MonkeyPatch) -> None:
    """只设通用版的变量不得把环境版一起打开，否则无法只启用其中一个。"""
    from cnki_search_env.professional_runtime import (
        WEBVPN_HOME_ENV,
        build_professional_runtime_from_env,
    )

    assert WEBVPN_HOME_ENV == "CNKI_ENV_WEBVPN_HOME"
    monkeypatch.setenv("CNKI_WEBVPN_HOME", "https://webvpn.example.edu.cn/")
    monkeypatch.delenv(WEBVPN_HOME_ENV, raising=False)
    with pytest.raises(ValueError, match=WEBVPN_HOME_ENV):
        asyncio.run(build_professional_runtime_from_env())


def test_env_runtime_never_imports_the_general_package() -> None:
    """产品隔离：环境版四个专业模块一律不得 import cnki_search。"""
    import ast
    from pathlib import Path

    root = Path(__file__).resolve().parents[1]
    for directory in ("scripts/cnki_search_env", "mcpb/src/cnki_search_env"):
        for name in ("professional.py", "professional_service.py",
                     "professional_runtime.py", "webvpn.py"):
            source = (root / directory / name).read_text(encoding="utf-8")
            for node in ast.walk(ast.parse(source)):
                if isinstance(node, ast.Import):
                    for alias in node.names:
                        assert not alias.name.startswith("cnki_search."), f"{name} 导入了 {alias.name}"
                        assert alias.name != "cnki_search", f"{name} 导入了通用版包"
                elif isinstance(node, ast.ImportFrom) and node.level == 0:
                    module = node.module or ""
                    assert module != "cnki_search" and not module.startswith("cnki_search."), (
                        f"{name} 从通用版包导入：{module}"
                    )
