...
from cnki_search_env.search import PageContractChanged
...
def test_both_tools_are_registered() -> None:
    assert mcp_server.REQUIRED_TOOLS == ["cnki_search_env", "cnki_professional_search_env"]
    mcp = CnkiMcpServer().build_fastmcp(RecordingMcp)
    assert set(mcp.tools) == {"cnki_search_env", "cnki_professional_search_env"}


def test_tool_description_states_the_human_attendance_requirement() -> None:
    """描述里必须写明约束，否则调用方会把它当成可随手调用的能力。"""
    mcp = CnkiMcpServer().build_fastmcp(RecordingMcp)
    description = mcp.tools["cnki_professional_search_env"]["description"]
    for statement in ("本人登录", "不可用于定时任务", "CNKI_ENV_WEBVPN_HOME"):
...
def test_disabled_webvpn_returns_configuration_error_without_launching_anything(
    ...
    result = asyncio.run(server.cnki_professional_search_env("数字经济"))
...
def test_concurrent_first_calls_build_and_close_exactly_one_runtime() -> None:
    async def scenario() -> None:
        ...
        second = asyncio.create_task(
            server.cnki_professional_search_env("共同富裕")
        )
...
def test_failed_factory_is_not_cached_and_next_call_can_retry() -> None:
    async def scenario() -> None:
        ...
        server = CnkiMcpServer(professional_factory=factory)
        first = await server.cnki_professional_search_env("数字经济")
        second = await server.cnki_professional_search_env("数字经济")
...
def test_english_priority_groups_are_refused_before_any_session_is_opened() -> None:
    ...
    result = asyncio.run(server.cnki_professional_search_env("主题", "ssci"))
    assert result["status"] == "configuration_error"
...
def test_arguments_are_passed_through_to_the_service() -> None:
    service = FakeProfessionalService()
    server = _server_with_service(service)
    asyncio.run(server.cnki_professional_search_env(
        "数字经济", "environment_cssci", 15, 2020, 2026))
    assert service.calls == [("数字经济", "environment_cssci", 15, 2020, 2026)]
...
def test_session_is_created_once_and_reused() -> None:
    ...
    server = CnkiMcpServer(professional_factory=factory)
    asyncio.run(server.cnki_professional_search_env("主题一"))
    asyncio.run(server.cnki_professional_search_env("主题二"))
...
def test_limit_matches_the_measured_page_size_ceiling() -> None:
    ...
    mcp = CnkiMcpServer().build_fastmcp(RecordingMcp)
    expected = {"cnki_search_env": 20, "cnki_professional_search_env": 50}
...
def test_shutdown_still_rejects_professional_calls() -> None:
    ...
    with pytest.raises(RuntimeError):
        asyncio.run(server.cnki_professional_search_env("主题"))
...
def test_professional_cancellation_propagates() -> None:
    ...
    with pytest.raises(asyncio.CancelledError):
        asyncio.run(server.cnki_professional_search_env("数字经济"))
...
def test_professional_year_schema_has_supported_bounds() -> None:
    ...
    tool = next(
        ...
        if item.name == "cnki_professional_search_env"
    )
...
@pytest.mark.parametrize("group", list(("chinese_environment_top", "other_formally_recognized_chinese",
                          "environment_cssci", "pku_core")))
def test_every_controlled_group_reaches_the_service(group: str) -> None:
    service = FakeProfessionalService()
    server = _server_with_service(service)

    asyncio.run(server.cnki_professional_search_env("环境政策", group))
