...
用途是在改动 :mod:`cnki_search_env.webvpn` 的选择器之前，先确认站点结构是否仍与
...
from cnki_search_env.professional import build_expression
...
async def run_probe(config: WebVpnConfig, *, submit_topic: str | None = None,
    ...
    """结构探测；``submit_topic`` 非空时额外提交一次 6 本环境顶刊的定向检索。"""
    async with session_factory(config) as session:
        ...
        if submit_topic:
            expression = build_expression(submit_topic, _CHINESE_ENVIRONMENT_TOP_JOURNALS)
    ...
    if failures:
        return ProbeResult(1, payload, summary,
                           "页面结构与 cnki_search_env.webvpn 中的契约不一致，需要更新选择器。")
    return ProbeResult(0, payload, summary)


def _load_chinese_environment_top_journals() -> list[str]:
    from catalog_lookup import journals_by_group
    return journals_by_group("chinese_environment_top")


_CHINESE_ENVIRONMENT_TOP_JOURNALS = _load_chinese_environment_top_journals()
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--submit", metavar="TOPIC", default=None,
                        help="额外提交一次 6 本中文环境顶尖期刊的定向检索（会消耗限流预算）")
