...
class PostSearchChallengePage(RestrictedPage):
    """首页正常，提交检索后被风控拦到安全验证页。

    这是 2026-07 的真实现象：知网首页可用，点击检索后跳转
    kns.cnki.net/verify/home?captchaType=blockPuzzle。
    """
...
def test_session_reports_challenge_instead_of_contract_change_after_search() -> None:
    """结果契约未出现时，必须先判断是不是站点主动阻断。

    两者补救方式相反：页面结构变化要改解析器，安全验证必须立即停手并改用
    其他检索途径。若一律报 page_contract_changed，排查会被带向错误方向。
    """
    page = PostSearchChallengePage()
    context = _Closable()
    context.new_page = lambda: page  # type: ignore[attr-defined]
    browser = _Closable()
    browser.new_context = lambda **_kwargs: context  # type: ignore[attr-defined]
    ...
    class ChallengedRunner:
        """模拟：检索提交成功，但结果契约等待超时（实际是跳到了验证码页）。"""
    ...
    status = session_module.classify_public_search_state(**snapshot.state_arguments())
    assert status is SearchStatus.CHALLENGE_DETECTED
    assert context.closed and browser.closed
