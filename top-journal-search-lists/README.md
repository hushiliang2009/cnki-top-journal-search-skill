...
有校园网权限时可启用第二种模式：由**你本人**经所在机构官方 WebVPN 完成统一身份认证后，用知网专业检索定向检索中文期刊论文，覆盖第六级中文顶尖期刊（13 本，按刊名精确限定）与第九级 CSSCI（在学术期刊条件区勾选来源类别，含扩展版）。注意 `LY=` 并非严格精确匹配，返回期刊不保证全部落在清单内；不在目录中的记录会被如实标为空层级与 `manual_review_required=true`。
...
页面操作顺序固定为：知网首页、高级检索、专业检索、学术期刊、再次确认专业检索、设置出版年度和来源类别、填写表达式、提交检索。该调整不改变13本中文顶刊或CSSCI的检索范围。

**来源类别不是专业检索字段**：CSSCI（`P0209`）与北大核心（`P01`）只在选择学术期刊后显示的条件区中勾选，且必须在提交前生效；不写入表达式或 `LY=`。控件异常时返回 `page_contract_changed`，不会提交未筛选检索。

可检索字段为 `SU`、`TKA`、`TI`、`KY`、`AB`、`CO`、`FT`、`AU`、`FI`、`RP`、`AF`、`LY`、`RF`、`FU`、`CLC`、`SN`、`CN`、`DOI`、`QKLM`、`FAF`、`CF`。`YE` 不受支持，年份须通过出版年度起止控件设置。
...
1. 统一身份认证需要你的校园账号密码，程序不接触凭据；
2. WebVPN 票据是 session cookie，跨浏览器进程无效，因此登录与检索必须在同一进程内完成，**浏览器窗口需全程打开**（关窗等同登出）；
3. 连续请求触发安全验证时需要你手工完成滑块。

它使用一次性的非持久化浏览器上下文，不读取或复用日常浏览器的会话。请求携带你的实名机构身份，所在机构的使用规范与频率限制均适用；默认请求间隔 30 秒。

遇到安全验证时程序会暂停并提示，你完成滑动后从断点续跑，已完成批次不重跑；你选择放弃时会如实报告未完成批次，不把不完整结果当作完整结果。

`no_data_retry_later` 表示页面暂时无法给出数据，不等于空结果，也不能据此断言没有相关文献。公开模式仍以 ai4scholar 为主要来源的中文补充，只访问公开首页，不登录、不下载。
...
| MCP 服务 | `cnki-search` |
| MCP 工具 | `cnki_search(query, limit)`；`cnki_professional_search(topic, group, limit, year_from, year_to)` |

`limit`：公开检索最大 20（只读结果页默认那一页）；WebVPN 专业检索最大 50（会主动把结果页「显示」档位切到 50，实测档位为 10/20/50）。
...
当前通用版为 `0.5.2`。正式 Release 包含 `top-journal-search-lists_Skill.zip`、`cnki-search.mcpb` 和 `checksums.sha256`。安装前须先按 `checksums.sha256` 核验两个压缩包。通用版使用 `top-journal-search-lists`、`cnki-search` 和 `runtimes/cnki-search`；可与环境版 `top-journal-search-lists-env`、`cnki-search-env` 并存，二者互不覆盖。
