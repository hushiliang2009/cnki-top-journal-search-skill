...
有校园网权限时可启用第二种模式：由**你本人**经所在机构官方 WebVPN 完成统一身份认证后，用知网专业检索定向检索中文期刊论文，覆盖第六级中文顶尖期刊（13 本，按刊名精确限定）与第九级 CSSCI（用结果页「来源类别」分面勾选，含扩展版）。注意 `LY=` 并非严格精确匹配，返回期刊不保证全部落在清单内；不在目录中的记录会被如实标为空层级与 `manual_review_required=true`。
