...
知网对自动化访问有站点级安全策略，在本 Skill 的边界内（不登录、不使用用户浏览器配置文件、不改 User-Agent、不用代理、不做任何检测规避）**可能长期返回 `challenge_detected` 而始终无法取得结果**。这属于站点侧正常防护，不是安装故障，重装、重试或更换网络都不会改变。

因此：

- `challenge_detected` 不等于"该主题无中文文献"。此时改用 ai4scholar，并在检索说明中如实写明"CNKI 补充检索未能执行，中文近期文献可能覆盖不足"。
- 不得为绕过该策略而伪造 User-Agent、轮换代理、抹除自动化标志或复用用户已登录的浏览器——这些属于检测规避，违反上面的检索边界。
